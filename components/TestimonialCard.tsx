...
function SourceBadge({ t, className = "" }: { t: Testimonial; className?: string }) {
  return t.source === "google" ? <GoogleBadge className={className} /> : <LinkedInBadge className={className} />;
}

function SourceIcon({ t, className = "" }: { t: Testimonial; className?: string }) {
  return t.source === "google" ? <GoogleIcon className={className} /> : <LinkedInIcon className={className} />;
}

function sourceLabel(t: Testimonial) {
  return t.source === "google" ? "Google" : "LinkedIn";
}

function sourceTextClass(t: Testimonial) {
  return t.source === "google" ? "text-charcoal-deep" : "text-[#0A66C2]";
}
  ...
  if (variant === "carousel") {
    return (
      <article className="relative bg-paper border border-line-strong p-8 md:p-12 flex flex-col items-center text-center">
        <SourceBadge t={t} className="absolute top-6 right-6" />
        <Avatar t={t} size={64} className="w-16 h-16 text-lg" />
        <p className="mt-5 text-sm font-medium text-charcoal-deep">{t.name}</p>
        {t.title && <p className="mt-1 text-xs text-muted max-w-sm">{t.title}</p>}
        ...
        <div className="mt-8 flex items-center gap-3 text-xs text-muted">
          ...
          <span className={`inline-flex items-center gap-1.5 font-medium ${sourceTextClass(t)}`}>
            <SourceIcon t={t} className={t.source === "google" ? "" : "text-[#0A66C2] opacity-100"} />
            {sourceLabel(t)}
          </span>
        </div>
      </article>
      ...
      <article className="group relative shrink-0 w-[320px] sm:w-[360px] h-[280px] bg-paper border border-line-strong p-6 md:p-7 flex flex-col transition-colors duration-300 hover:border-slate">
        ...
        <header className="flex items-start gap-3 pr-10">
          ...
          <div className="min-w-0 pt-0.5">
            ...
            {t.title && <p className="mt-1 text-xs text-muted leading-snug line-clamp-2">{t.title}</p>}
          </div>
        </header>
        ...
        <footer className="mt-4 pt-4 border-t border-line-strong flex items-center justify-between gap-4">
          ...
          <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${sourceTextClass(t)}`}>
            <SourceIcon t={t} className={t.source === "google" ? "" : "text-[#0A66C2] opacity-100"} />
            {sourceLabel(t)}
          </span>
        </footer>
  ...
  if (variant === "masonry") {
    return (
      <article className="group relative mb-6 break-inside-avoid bg-paper border border-line-strong p-7 md:p-8 transition-all duration-300 hover:border-slate hover:shadow-[0_8px_30px_rgba(26,29,30,0.07)]">
        <SourceBadge t={t} className="absolute top-6 right-6 md:top-7 md:right-7" />
        <header className="flex items-start gap-4 pr-10">
          <Avatar t={t} size={52} className="w-[52px] h-[52px] text-base" />
          <div className="min-w-0 pt-0.5">
            <p className="text-sm font-medium text-charcoal-deep leading-tight">{t.name}</p>
            {t.title && <p className="mt-1 text-xs text-muted leading-snug">{t.title}</p>}
            {t.source === "google" && <StarRating className="mt-1.5" />}
          </div>
        </header>
        ...
        <footer className="mt-7 pt-5 border-t border-line-strong flex items-center justify-between gap-4">
          ...
          <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${sourceTextClass(t)}`}>
            <SourceIcon t={t} className={t.source === "google" ? "" : "text-[#0A66C2] opacity-100"} />
            {t.source === "google" ? "Google Review" : "LinkedIn Recommendation"}
          </span>
        </footer>
      </article>
    );
    ...
    <div className="bg-paper h-full">
      <article className="group relative bg-paper border border-line-strong p-7 md:p-8 transition-colors duration-300 hover:border-slate h-full flex flex-col">
        ...
        <div className="flex-1 flex flex-col justify-between mt-6">
          ...
          <footer className="mt-7 pt-5 border-t border-line-strong flex items-center justify-between gap-4">
            <span className="text-xs text-muted">{t.date}</span>
            <span className="inline-flex items-center gap-1.5 text-xs text-muted" aria-label={`Sourced from ${sourceLabel(t)}`}>
              <span>via {sourceLabel(t)}</span>
              <SourceIcon t={t} />
            </span>
          </footer>
        </div>
